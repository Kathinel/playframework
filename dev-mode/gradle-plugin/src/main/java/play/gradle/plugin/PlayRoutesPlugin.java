...
public class PlayRoutesPlugin implements Plugin<Project> {
  ...
  private void configureDefaultSettings(final Project project) {
    ...
    playExtension.getRoutes().getNamespaceReverseRouter().convention(false);
    playExtension.getRoutes().getGenerateReverseRouter().convention(true);
    playExtension
        .getRoutes()
        .getImports()
        .addAll(
  ...
  private Configuration createDefaultRoutesConfiguration(Project project) {
  ...
  private void configureSourceSetDefaults(
    ...
    javaPluginExtension(project)
        ...
        .all(
            (sourceSet) -> {
              SourceDirectorySet routesSource = getRoutesSourceDirectorySet(sourceSet);
  ...
  private SourceDirectorySet getRoutesSourceDirectorySet(SourceSet sourceSet) {
    String displayName = ((DefaultSourceSet) sourceSet).getDisplayName();
    SourceDirectorySet routesSource =
        objectFactory.sourceDirectorySet("routes", displayName + " Play Routes source");
