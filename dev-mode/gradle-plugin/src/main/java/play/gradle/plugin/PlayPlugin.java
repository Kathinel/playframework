...
public class PlayPlugin implements Plugin<Project> {
  ...
  private void configureTwirlDefaultImports(final Project project) {
    ...
    for (SourceSet sourceSet : List.of(mainSourceSet(project), testSourceSet(project))) {
      Task twirlCompile = project.getTasks().findByName(sourceSet.getCompileTaskName("twirl"));
      if (twirlCompile == null) continue;
      if (twirlCompile.hasProperty("templateImports")) {
        var imports = (SetProperty<String>) twirlCompile.property("templateImports");
        if (imports != null) {
          imports.addAll(
              () -> {
      ...
      if (twirlCompile.hasProperty("constructorAnnotations")) {
        var annotations = (ListProperty<String>) twirlCompile.property("constructorAnnotations");
