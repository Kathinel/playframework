...
public class PlayRunPlugin implements Plugin<Project> {
  ...
  private boolean isProjectComponent(ComponentIdentifier component) {
  ...
  private List<String> filterProjectComponents(Configuration configuration) {
    var projects = new ArrayList<String>();
    configuration
        .getIncoming()
        .artifactView(view -> view.componentFilter(this::isProjectComponent))
        .getArtifacts()
        .forEach(
            artifact ->
                projects.add(
  ...
  FileCollection filterNonChangingArtifacts(Configuration configuration) {
